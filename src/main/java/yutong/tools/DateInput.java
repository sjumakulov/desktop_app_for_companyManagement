...
public class DateInput extends JPanel {
    ...
    public DateInput(String ID, JPanel panel, Connection connection, int fontSize) {
        ...
        yyyy.addKeyListener(search(dd, MM, yyyy, ID, panel, connection, fontSize));
        MM.addKeyListener(search(dd, MM, yyyy, ID, panel, connection, fontSize));
        dd.addKeyListener(search(dd, MM, yyyy, ID, panel, connection, fontSize));
    ...
    private static KeyListener search(JTextField dd, JTextField MM, JTextField yyyy, String ID, JPanel panel, Connection connection, int fontSize) {
        return new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if (DAY.length() == 2 && MONTH.length() == 2 && YEAR.length() == 4) {
                    list = getData("SELECT * FROM " + ID + " WHERE KUN= '" + DAY + "' AND OY='" + MONTH + "' AND YIL='" + YEAR + "'", connection);
                    update = true;
                } else if (MONTH.length() == 2 && YEAR.length() == 4) {
                    list = getData("SELECT * FROM " + ID + " WHERE OY='" + MONTH + "' AND YIL='" + YEAR + "'", connection);
                    update = true;
                } else if (YEAR.length() == 4) {
                    list = getData("SELECT * FROM " + ID + " WHERE YIL='" + YEAR + "'", connection);
                    update = true;
                } else if (DAY.length() == 0 && MONTH.length() == 0 && YEAR.length() == 0) {
                    list = getData("SELECT * FROM " + ID + " WHERE YIL='" + YIL + "'", connection);
                    update = true;
                }
                ...
                if (update && !list2.equals(list) || firstTime) {
                    ...
                    for (Object p : list) {
                        ...
                        panel.add(penaltyMessage(DATE, valueOf(penaltyMap, "TEXT"), valueOf(penaltyMap, "ID"), ID, connection, fontSize), gbc);
                    }
                    panel.revalidate();
                    panel.repaint();
                }
